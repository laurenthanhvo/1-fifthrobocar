...
class LaneCenterFollower:
    def __init__(self, pid, cfg):
        ...
        # ============================================================
        # OAK-D depth / obstacle visualization
        # ============================================================

        self.depth_view_mode = getattr(cfg, "DEPTH_VIEW_MODE", "off")
        self.depth_obstacle_enable = getattr(cfg, "DEPTH_OBSTACLE_ENABLE", False)

        self.depth_min_mm = getattr(cfg, "DEPTH_OBSTACLE_MIN_MM", 150)
        self.depth_max_mm = getattr(cfg, "DEPTH_OBSTACLE_MAX_MM", 1800)
        self.depth_stop_distance_mm = getattr(cfg, "DEPTH_STOP_DISTANCE_MM", 400)

        self.depth_min_obstacle_area = getattr(cfg, "DEPTH_MIN_OBSTACLE_AREA", 700)
        self.depth_roi_y_start = getattr(cfg, "DEPTH_ROI_Y_START", 0.20)
        self.depth_roi_y_end = getattr(cfg, "DEPTH_ROI_Y_END", 1.00)
        self.depth_corridor_half_width_px = getattr(
            cfg,
            "DEPTH_CENTER_CORRIDOR_HALF_WIDTH_PX",
            220,
        )

        self.depth_fx_px = getattr(cfg, "DEPTH_FX_PX", 287.82)
        self.depth_fy_px = getattr(cfg, "DEPTH_FY_PX", 287.82)
        self.depth_heatmap_alpha = getattr(cfg, "DEPTH_HEATMAP_ALPHA", 0.45)

        self.depth_confirm_frames = getattr(cfg, "DEPTH_CONFIRM_FRAMES", 2)
        self.depth_seen_count = 0
    ...
    # ============================================================
    # Depth obstacle visualization
    # ============================================================

    def _resize_to(self, arr, w, h, is_depth=False):
        if arr is None:
            return None
    ...
    def _make_depth_heatmap_from_depth(self, depth_mm):
        min_mm = max(1, int(self.depth_min_mm))
        max_mm = max(min_mm + 1, int(self.depth_max_mm))
        ...
        norm = ((max_mm - depth) / float(max_mm - min_mm) * 255.0)
    ...
    def _detect_depth_obstacle(self, depth_mm, lane_center_x):
        """
        Detect close depth blobs.

        Returns:
            bbox, distance_mm, width_m, height_m
        or:
            None
        """
        h, w = depth_mm.shape[:2]

        y0 = int(h * self.depth_roi_y_start)
        y1 = int(h * self.depth_roi_y_end)
        ...
        half = int(self.depth_corridor_half_width_px)
        x0 = max(0, cx - half)
        x1 = min(w, cx + half)
        ...
        valid = (
            (roi_depth >= int(self.depth_min_mm))
            & (roi_depth <= int(self.depth_max_mm))
        ).astype(np.uint8) * 255
        ...
        for c in contours:
            ...
            if area < float(self.depth_min_obstacle_area):
                continue
            ...
            obj_depth = depth_mm[full_y:full_y + bh, full_x:full_x + bw]
            obj_valid = obj_depth[
                (obj_depth >= int(self.depth_min_mm))
                & (obj_depth <= int(self.depth_max_mm))
            ]
            ...
            distance_mm = float(np.median(obj_valid))

            distance_m = distance_mm / 1000.0
            width_m = (float(bw) * distance_m) / max(float(self.depth_fx_px), 1.0)
            height_m = (float(bh) * distance_m) / max(float(self.depth_fy_px), 1.0)
        ...
        # Choose the closest valid obstacle.
        candidates.sort(key=lambda d: d["distance_mm"])
        return candidates[0]

    def _apply_depth_view(
        self,
        raw_img,
        debug_img,
        depth_mm,
        ...
        """
        Draw depth heatmap + obstacle bbox/distance/size into cv/image_array.
        Can also stop the car if obstacle is close enough.
        """
        mode = str(self.depth_view_mode).lower()

        if mode == "off" or depth_mm is None:
            return debug_img, steering, throttle

        raw_h, raw_w = raw_img.shape[:2]

        depth_raw = self._resize_to(depth_mm, raw_w, raw_h, is_depth=True)

        if depth_heatmap is None:
            heatmap_raw = self._make_depth_heatmap_from_depth(depth_raw)
        else:
            heatmap_raw = self._resize_to(depth_heatmap, raw_w, raw_h, is_depth=False)

        # ------------------------------------------------------------
        # Mode 1: accurate raw depth camera view
        # ------------------------------------------------------------
        if mode == "depth_only":
            out_img = cv2.addWeighted(
                raw_img,
                1.0 - float(self.depth_heatmap_alpha),
                heatmap_raw,
                float(self.depth_heatmap_alpha),
                0,
            )
        ...
        # ------------------------------------------------------------
        # Mode 2: overlay depth heatmap on current lane debug view
        # This is useful visually, but if current debug is bird's-eye,
        # the object size box is still calculated from raw depth.
        # ------------------------------------------------------------
        else:
            dbg_h, dbg_w = debug_img.shape[:2]

            heatmap_for_debug = self._resize_to(
                heatmap_raw,
                dbg_w,
                dbg_h,
                is_depth=False,
        ...
        if not self.depth_obstacle_enable:
            return out_img, steering, throttle

        obstacle = self._detect_depth_obstacle(depth_raw, lane_center_x)

        # Draw search corridor on raw/depth_only view.
        if mode == "depth_only":
            h, w = out_img.shape[:2]
            cx = int(lane_center_x) if lane_center_x is not None else w // 2
            half = int(self.depth_corridor_half_width_px)
            ...
            cv2.rectangle(out_img, (x0, y0), (x1, y1), (255, 128, 0), 1)
        ...
        x, y, bw, bh = obstacle["bbox"]
        distance_mm = obstacle["distance_mm"]
        distance_m = distance_mm / 1000.0
        width_m = obstacle["width_m"]
        height_m = obstacle["height_m"]

        close_enough = distance_mm <= float(self.depth_stop_distance_mm)

        if close_enough:
            self.depth_seen_count += 1
        else:
            self.depth_seen_count = 0

        should_stop = self.depth_seen_count >= int(self.depth_confirm_frames)

        if mode == "depth_only":
            # Bbox is accurate in raw/depth_only mode.
            cv2.rectangle(
                out_img,
                (x, y),
                (x + bw, y + bh),
            ...
            label1 = f"OBSTACLE dist={distance_m:.2f}m"
            label2 = f"w={width_m:.2f}m h={height_m:.2f}m"
            ...
            # In overlay mode, just show text because bbox may not align
            # if debug_img is bird's-eye.
            label = (
                f"DEPTH OBSTACLE dist={distance_m:.2f}m "
    ...
    def run(self, img, depth_mm=None, depth_heatmap=None):
        if img is None:
            return 0.0, 0.0, None
        ...
        # OAK-D depth visualization + depth-based obstacle stop.
        debug_img, steering, throttle = self._apply_depth_view(
            img,
            debug_img,
            depth_mm,
            depth_heatmap,
