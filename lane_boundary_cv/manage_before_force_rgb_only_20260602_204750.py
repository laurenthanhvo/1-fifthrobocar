...
def drive(cfg, use_joystick=False, camera_type='single', meta=[]):
    ...
    V.add(OakCameraDepth(
      ...
      outputs=["cam/image_array", "cam/depth_array"],  # now produces depth too
      threaded=True,
)
    ...
    # ===== INSERT BLOCK 1: import at the top of the file (with other imports) =====
    # from obstacle_avoidance import ObstacleDetector, ObstacleAvoider
 
    # # ===== INSERT BLOCK 2: add parts here, right after add_cv_controller =====
    # #
    # # Obstacle Detection
    # # Reads the camera image (+ depth if available) and publishes detection state
    # #
    # obstacle_detector = ObstacleDetector(cfg)
    # V.add(
    #     obstacle_detector,
    #     inputs=['cam/image_array', 'cv/image_array'],          # add 'cam/depth_array' once OakCamera exposes it
    #     outputs=['obstacle/detected', 'obstacle/distance', 'obstacle/position', 'cv/image_array'],
    #     run_condition='run_pilot'            # only runs in autopilot mode, same as lane follower
    # )
 
    # #
    # # Obstacle Avoidance
    # # Reads pilot/steering + pilot/throttle FROM the lane follower
    # # and OVERWRITES them in vehicle memory if an obstacle is detected.
    # # Must come before DriveMode so DriveMode picks up the corrected values.
    # #
    ...
    #     outputs=['pilot/steering', 'pilot/throttle'],  # overwrites in-place
    #     run_condition='run_pilot'
    # )
    # ===== END INSERT =====

    # ── YOLO + depth obstacle detection ──────────────────────────────
    from obstacle_avoidance_yolo import YoloLidarObstacleDetector
    from obstacle_avoidance import ObstacleAvoider

    V.add(
        YoloLidarObstacleDetector(cfg),
        inputs=['cam/image_array', 'cam/depth_array', 'cv/image_array'],
        outputs=['obstacle/detected', 'obstacle/distance', 'obstacle/position', 'cv/image_array'],
        run_condition='run_pilot',
    )
    V.add(
...
#             fps=cfg.DRIVE_LOOP_HZ,
#             undistort=getattr(cfg, "OAK_UNDISTORT", False),
#             undistort_model=getattr(cfg, "OAK_UNDISTORT_MODEL", "auto"),
#             undistort_alpha=getattr(cfg, "OAK_UNDISTORT_ALPHA", 0.0),
#         ),
#         outputs=['cam/image_array'],
#         threaded=True
