...
## 6) Navigation Roadmap (Next Steps)

This section outlines the high-level step-by-step plan for getting from the current working car setup to a basic navigation pipeline.

### Current status (what is already working)

Based on the progress so far, we already have:

- Manual driving working through DonkeyCar / controller flow
- Camera access working
- LiDAR topic publishing working
- Foxglove bridge working for visualization
- Raw GPS data access partially working
- Camera/LiDAR calibration workflow started
- Path-following infrastructure already present in the car codebase

This means the next focus should be **navigation**, not just bring-up.

---

### 6.1 Freeze one working baseline

Before adding more features, make sure the team is using one consistent software setup.

Goal:

- One agreed-upon ROS environment
- One agreed-upon launch sequence
- Stable topic names
- Stable USB / device mappings
- Stable frame naming

Why this matters:

- If different people are launching different versions of the stack, navigation debugging becomes very difficult.
- We want everyone testing on the same baseline before building autonomy.

Deliverable:

- One repeatable startup flow that consistently brings up the sensors and visualization stack

---

### 6.2 Define the navigation pipeline

Before implementation, clearly define the full system pipeline:

1. Sensor input
2. Localization
3. Path planning / path following
4. Low-level control
5. Safety layer

At a high level, the car needs to answer three questions:

- Where am I?
- Where am I trying to go?
- Is it safe to keep moving?

Deliverable:

- A simple architecture diagram or written description of what each stage publishes / consumes

---

### 6.3 Get localization working first

Localization should be the next major milestone.

The car needs a usable estimate of its position and heading before reliable navigation is possible.

Inputs that may be used:

- Wheel / drivetrain motion estimate
- GPS
- IMU
- LiDAR / future perception inputs if needed

Main objective:

- Produce a consistent pose estimate for the car over time

Deliverable:

- A ROS topic representing the vehicle pose
- A valid TF chain connecting the major frames

Notes:

- Do not move on to full navigation until the pose estimate is stable enough to visualize and trust
- Manual driving tests should be used to validate localization first

---

### 6.4 Set up frames, TF, and visualization

Once localization is being published, verify that everything is aligned correctly in visualization.

At minimum, the following frames should be clearly defined:

- `map`
- `odom`
- `base_link`
- sensor frames such as LiDAR / camera / GPS frames

Goal:

- Be able to open Foxglove and understand where the car thinks it is, how it is moving, and where the sensors are mounted

Deliverable:

- A clean TF tree and a visualization setup that can be reused across tests

---

### 6.5 Build a simple waypoint-following MVP

Do not jump directly into complex autonomy.

The first navigation milestone should be:

- Follow a prerecorded path
- Or follow a list of waypoints outdoors at low speed

This is the simplest useful navigation demo and gives the team a concrete target.

Recommended approach:

1. Record a route or waypoint list
2. Use localization to estimate current position
3. Compute steering / throttle commands to track the route
4. Send commands through the existing drive interface

Deliverable:

- The car can drive along a known route with minimal human intervention

---

### 6.6 Add a LiDAR-based safety layer

Before doing longer autonomous runs, add a simple safety layer using LiDAR.

Initial safety behavior can be simple:

- Slow down if an obstacle is nearby
- Stop if an obstacle is too close
- Optionally refuse motion if localization is clearly invalid

This does not need to be a full obstacle-avoidance planner yet.

Goal:

- Prevent the car from driving into something during early testing

Deliverable:

- A safety node or safety check in the control pipeline that can override motion commands when necessary

---

### 6.7 Test in stages

Testing should be done in increasing levels of difficulty.

Suggested order:

1. Validate sensors and TF while stationary
2. Validate localization while manually driving
3. Test waypoint following in a wide open area
4. Test waypoint following with LiDAR safety enabled
5. Test longer outdoor runs
6. Only after that, consider richer autonomy / obstacle-aware navigation

Why this matters:

- If something breaks, staged testing makes it easier to identify whether the issue is localization, control, sensor timing, or safety logic

---

### 6.8 Log runs and tune the system

Every navigation test should be recorded and reviewed.

Things to inspect after each run:

- Did localization drift?
- Did the controller oscillate?
- Did the car miss waypoints?
- Did the safety stop trigger correctly?
- Were there topic dropouts or timing issues?

Goal:

- Improve one bottleneck at a time rather than changing everything at once

Deliverable:

- A repeatable tuning workflow for improving navigation performance

---

### 6.9 Only after that, add richer perception

After basic navigation works, the team can decide whether to invest further in:

- Camera-LiDAR fusion for navigation
- Vision-based lane following
- Obstacle classification
- More advanced local planning
- Semantic perception / VLM-based behaviors

Important:

- These are valuable future directions, but they should not block the first navigation milestone
